...
-- added row number to count duplicates in each app
...
main AS (-- main outer query to determine columns and total net profit
SELECT DISTINCT TRIM(name) AS name,
        rating,
        store,
        cost,
...
SELECT category,
	COUNT(category),
	(COUNT(category))/SUM(COUNT(*)) OVER () AS cat_perc,
	SUM(total_net_profit) AS total_net_profit,
	AVG(rating) AS avg_rating
FROM main
GROUP BY category
ORDER BY cat_perc DESC
--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*;